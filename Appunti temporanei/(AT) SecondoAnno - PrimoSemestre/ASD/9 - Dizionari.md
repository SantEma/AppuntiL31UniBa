Abbiamo compreso come con l'utilizzo degli insiemi possiamo di fatto utilizzare l'algebra degli insiemi per operare su strutture di dati che necessitano le operazione di unione, intersezione, ecc...
Ma esistono in informatica molte applicazioni che necessitano una struttura come gli **insiemi** ma che non richiedono l'uso delle loro funzione tipiche, per questo nascono i **dizionari**, definiti come **sottotipi del tipo di dato astratto del padre** (ovvero la classe degli insiemi).
...
- **Accesso**: permette di entrare nelle informazioni del valore associato ad una chiave per aggiornale o estrapolarle.
...
- `inserisci(<k,v>,D)=D'`, l'operazione di inserimento gestisce due casi distinti, l'inserimento **puro** o l'inserimento di **aggiornamento**.
	  ...
	  - **Caso di una chiave esistenti**: se esiste giù una coppia $<k',v' | k'=k>$, questa viene rimossa e sostituita con la nuova coppia. $$D'=D\setminus<k',v'>\bigcup<k,v>$$
...
Le tipologie della forma hash si divide in dinamica e statica, noi affronteremo lo studio di quella **statica**, concentrandoci quindi sulla dimensione fissa.
...
Basandosi sulle nozioni precenti, non esiste un "migliore" assoluto, ma la scelta dipende dalle esigenze specifiche del programma (memoria vs flessibilità).
...
- **Estrazione**:
  ...
  **Contro**: se le chiavi hanno pattern simili nelle posizioni scelte si andrà in contro alle collisioni.
...
La conseguenza data da questa chiave fittizia è la **degradazione del tempo di ricerca**. Nel caso la tabella fosse sotto posta a molte cancellazioni, la C.C. diventa lineare ($O(m)$), rendendo necessario ristrutturare totalmente la tabella.
...
![[Trabocco.png]]
Nell'esempio si nota che tutti gli elementi che puntano alla cella $16$ saranno puntatori a loro volta degli altri elementi successivi che iniziano per P, richiedenti anche loro il valore 16.
...
Il **vettore ordinato** rispetto ad un insieme generico ha la possibilità di eseguire una ricerca **binaria** su di esso, grazie all'ordinamento delle chiavi contigue, questo ci fornisce una C.C. pari a $O(\log\ n)$, ovvero al raddoppiare dei dati, il tempo di ricerca cresce di un unità.
Inoltre, in un vettore combatto la cancellazione/inserimento di dati nel vettore richiede lo spostamento di tutti gli elementi successivi per mantenere un determinato ordine; questo rende le modifiche molto **costose** rispetto alla tabella hash.
