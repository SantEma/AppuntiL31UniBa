...
1. **REGOLA 1**: Supponiamo che il programma sia composto da due parti P e Q da eseguire **sequenzialmente** e che i costi di P e Q siano:
   $$S(n)=O(f(n)) e T(n)=O(g(n))$$Il costo del programma è $O(max(f(n), g(n)))$
2. **REGOLA 2**: La seconda regola ci permette di valutare il costo del programma quando esso richiede **più volte l'esecuzione di un insieme di istruzioni** o l'attivazione di una procedura.
   Supponiamo che un programma richieda k volte l'esecuzione di una istruzione composta o l'attivazione di una procedura, e sia $f_{i}(n)$ il costo relativo all'esecuzione i-esima, i=1,2...k. Il costo complessivo del programma è pari a: 
   $$O\left( \sum_{i} f_{i}(n) \right)$$
